...
Eight project subagents live in `.claude/agents/` (generated from canonical specs in
`.agents/agents/`). Invoke one with `@<name>` or just describe the need and let Claude route by
each agent's `description`:
`game-coordinator` (PM/decompose) → `game-designer` → `engine-architect` →
`gameplay-programmer` (+ `narrative-writer`, `balance-economist` in parallel) →
`qa-tester` → `code-reviewer`. Subagents can't spawn each other, so `game-coordinator`
returns a delegation plan for the main thread to execute step by step.
